...
__global__
void
div_diagonal_kernel(T* input, T* value, int64_t flatten_dim, int64_t step, int64_t len){
...
__global__
void
div_diagonal_array_kernel(T* input, T* values, int64_t flatten_dim, int64_t step, int64_t len){
    int64_t thread_index = blockIdx.x * blockDim.x + threadIdx.x + THREAD_COUNT * BLOCK_COUNT * step;
    if (thread_index >= len) return;
    T* local_input = input;
...
torch::Tensor
div_diagonal(torch::Tensor input, torch::Tensor value){
    ...
    if(!value.dim()){
        ...
        AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::BFloat16, input.scalar_type(), "div_diagonal", [&]{
            ...
            for(int64_t i = 0; i < len + STEP_DIM; i += STEP_DIM)
                div_diagonal_kernel<scalar_t><<<BLOCK_COUNT, THREAD_COUNT, 0, stream>>>(input_ptr, scalar_val, flatten_dim, i / STEP_DIM, len);
            return input;
        });
    }else{
        ...
        AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::BFloat16, input.scalar_type(), "div_diagonal", [&]{
            ...
            for(int64_t i = 0; i < len + STEP_DIM; i += STEP_DIM)
                div_diagonal_array_kernel<scalar_t><<<BLOCK_COUNT, THREAD_COUNT, 0, stream>>>(input_ptr, value_ptr, flatten_dim, i / STEP_DIM, len);
            return input;
        });
    }
